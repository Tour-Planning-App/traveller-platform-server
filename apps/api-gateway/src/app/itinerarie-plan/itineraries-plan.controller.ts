...
export class ItinerariesPlanController {
  ...
  async createTrip(@Body() dto: CreateTripDto, @Req() req: any) {
    ...
    try {
      const result: { trip: any } = await firstValueFrom(
        this.itinerariesService.CreateTrip({ user_id: userId, ...dto }).pipe(
          catchError((error) => {
  ...
  async updateTrip(@Param('id') id: string, @Body() dto: UpdateTripDto, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.UpdateTrip({ id, user_id: userId, ...dto }).pipe(
          catchError((error) => {
  ...
  async getUserTrips(@Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.GetUserTrips({ user_id: userId }).pipe(
          catchError((error) => {
  ...
  async deleteTrip(@Param('id') id: string, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.DeleteTrip({ id, user_id: userId }).pipe(
          catchError((error) => {
  ...
  async addItineraryItem(@Param('id') tripId: string, @Param('day') day: number, @Body() dto: AddItineraryItemDto, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.AddItineraryItem({ trip_id: tripId, user_id: userId, day: day.toString(), ...dto }).pipe(
          catchError((error) => {
  ...
  async removeItineraryItem(@Param('id') tripId: string, @Param('day') day: number, @Param('activityId') activityId: string, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.RemoveItineraryItem({ trip_id: tripId, user_id: userId, day: day.toString(), activity_id: activityId }).pipe(
          catchError((error) => {
  ...
  async addBucketItem(@Param('id') tripId: string, @Body() { name, description }: { name: string; description: string }, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.AddBucketItem({ trip_id: tripId, user_id: userId, name, description }).pipe(
          catchError((error) => {
  ...
  async removeBucketItem(@Param('id') tripId: string, @Param('itemId') itemId: string, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.RemoveBucketItem({ trip_id: tripId, user_id: userId, item_id: itemId }).pipe(
          catchError((error) => {
  ...
  async shareTrip(@Param('id') tripId: string, @Req() req: any) {
    ...
    try {
      const result : any= await firstValueFrom(
        this.itinerariesService.ShareTrip({ trip_id: tripId, user_id: userId }).pipe(
          catchError((error) => {
  ...
  async addNote(@Param('id') tripId: string, @Param('day') day: number, @Body() { content }: { content: string }, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.AddNote({ trip_id: tripId, user_id: userId, day: day.toString(), content }).pipe(
          catchError((error) => {
  ...
  async addChecklistItem(@Param('id') tripId: string, @Param('day') day: number, @Body() { text }: { text: string }, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.AddChecklistItem({ trip_id: tripId, user_id: userId, day: day.toString(), text }).pipe(
          catchError((error) => {
  ...
  async updateChecklistItem(@Param('id') tripId: string, @Param('day') day: number, @Param('itemId') itemId: string, @Body() { completed }: { completed: boolean }, @Req() req: any) {
    ...
    try {
      const result = await firstValueFrom(
        this.itinerariesService.UpdateChecklistItem({ trip_id: tripId, user_id: userId, day: day.toString(), item_id: itemId, completed }).pipe(
          catchError((error) => {
